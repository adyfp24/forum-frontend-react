...
function Beranda() {
  ...
  const showGroup = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/v1/group', {headers});
      if(response.status === 200){
        setDataGrup(response.data.group || []);
      }else{
        console.log('tidak ada data')
      }
    } catch (error) {
      alert('gagal dapat grup' + error)
    }
  }
        ...
        <div className="flex flex-col w-full text-gray-800">
          ...
          <main className="p-6 sm:p-10 space-y-6 overflow-y-auto">
            ...
            <section className="flex flex-col w-full">
              {dataGrup.map((data, id) => (
              <button key={id} className="bg-white shadow rounded-lg w-full mb-6 hover:drop-shadow-lg">
                <div className="flex items-center justify-between px-6 py-5 font-semibold border-b border-gray-100">
